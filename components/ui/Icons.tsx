...
export const DeliaLogo: React.FC<{className?: string, isCollapsed?: boolean}> = ({ className, isCollapsed }) => (
    <div className={`relative ${className} flex items-center`}>
        <svg viewBox="0 0 68 80" className="h-full w-auto flex-shrink-0" preserveAspectRatio="xMinYMid meet">
            <rect x="10" y="10" width="50" height="50" transform="rotate(45 35 35)" className="fill-brand-purple"/>
            <text x="22" y="50" fontFamily="Arial, sans-serif" fontWeight="bold" fontSize="36" fill="white">D</text>
        </svg>
        <div className={`overflow-hidden ${isCollapsed ? 'lg:w-0' : 'w-auto'}`}>
             <span className="font-bold text-3xl text-brand-purple pl-1">elia</span>
        </div>
    </div>
);
